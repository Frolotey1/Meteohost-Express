...
{
    internal class MetarService : BackgroundService
    {
        ...
        public MetarService(IAirports airports, LoadService loadService, ILogger<MetarService> logger)
        {
            _logger = logger;
            _airports = airports;
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            _logger.LogInformation("meteo integration started [{IataList}]", string.Join(',', arr));
        ...
        private async Task StartAirportAsync(string iata, CancellationToken cancellationToken)
        {
            ...
            while (true)
                try
                {
                    await _loadService.LoadMetarAsync(iata);
                ...
                {
                    _logger.LogWarning(ex, "meteo api error {Iata}", iata);
                }
                catch (Exception e)
                {
                    _logger.LogError(e, "unhandled error, wait 10 mins {Iata}", iata);
